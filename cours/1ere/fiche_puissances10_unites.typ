...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  [
    #définition[
      *Puissances positives* ($10^n$, $n gt.eq 1$). $10^n$ = 10 × 10 × ... × 10 ($n$ fois) : un 1 suivi de $n$ zéros. Cas particulier : $10^0 = 1$.
    ]
    #exemple[
      $10^1 = 10$ #h(0.4cm) $10^3 = 1 space 000$ #h(0.4cm) $10^5 = 100 space 000$
    ]
  ],
  [
    #définition[
    ...
    #exemple[
      $10^(-1) = 1/10 = 0,1$ #h(0.4cm) $10^(-2) = 1/100 = 0,01$
    ]
  ]
)
...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  ...
  exemple[
    ...
    3 500 = 3,5 × $10^3$ #h(0.3cm) 0,0042 = 4,2 × $10^(-3)$
  ]
)
  ...
  [
    #définition[
      *Méthode.* 1. Déplacer la virgule pour un seul chiffre non nul avant. #linebreak()
      ...
      3. Vers la *gauche* → exposant *positif*. #linebreak()
      4. Vers la *droite* → exposant *négatif*.
    ]
    #exemple[
...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  définition[
    $10^a times 10^b = 10^(a+b)$ — on *additionne* les exposants. #linebreak() $10^a/10^b = 10^(a-b)$ — on *soustrait* les exposants.
  ],
  exemple[
...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  définition[
    $1/10^a = 10^(-a)$ — on *change le signe* de l'exposant. #linebreak() $(10^a)^b = 10^(a times b)$ — on *multiplie* les exposants.
  ],
  exemple[
...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  ...
  exemple[
    *Ex. 2 — Intensité.* Convertir 470 µA en A. #linebreak()
    Préfixe *µ* = $10^(-6)$ → 470×$10^(-6)$ A = *4,70×$10^(-4)$ A*
  ]
)

#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  définition[
    *Méthode B — entre deux préfixes.* On passe toujours par l'unité de base comme étape intermédiaire. Raccourci : l'exposant du résultat = exposant initial − exposant final.
  ],
  exemple[
...
#grid(columns: (1fr, 1fr), column-gutter: 8pt,
  ...
  exemple[
    *Ex. 2 — depuis un préfixe du litre.* Convertir 50 mL en m³. #linebreak()
    1 mL = $10^(-3)$ L = $10^(-6)$ m³ → 50 mL = *5×$10^(-5)$ m³*
  ]
)
...
[
#text(weight: "bold", size: 11pt)[Partie 1 — Puissances de 10]
#v(4pt)

*Définitions et écritures*
+ 10 000
+ 1 000 000
+ 0,01
+ 0,0001
...
[
#text(weight: "bold", size: 11pt)[Partie 2 — Convertir les unités]
#v(4pt)
