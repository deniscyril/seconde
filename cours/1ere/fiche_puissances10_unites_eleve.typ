...
#rappel[Puissances positives : $10^n$ ($n gt.eq 1$)][
  ...
  Cas particulier : $10^0 =$ #blanc(w: 1.2cm).
]
...
#exemple[Exemple détaillé][
  ...
  $10^(-2) = 1/100 =$ #blanc(w: 1.6cm) #linebreak()
//  $10^(-3) = 1/1000 = 0,001$
]
...
#exemple[Exemple détaillé][
  ...
  0,0042 = 4,2 × $10^(-3)$ (3 rangs, puissance #blanc(w: 2cm))
]
...
#rappel[Multiplication][
  $10^a times 10^b = 10^(a+b)$ — on #blanc(w: 2.6cm) les exposants.
]
#exemple[Exemple détaillé][
...
#rappel[Division][
  $10^a/10^b = 10^(a-b)$ — on #blanc(w: 2.6cm) les exposants.
]
#exemple[Exemple détaillé][
...
#rappel[Inverse][
  $1/10^a = 10^(-a)$ — on #blanc(w: 3.4cm) de l'exposant.
]
#exemple[Exemple détaillé][
...
#rappel[Puissance d'une puissance][
  $(10^a)^b = 10^(a times b)$ — on #blanc(w: 2.6cm) les exposants.
]
#exemple[Exemple détaillé][
...
#rappel[Méthode B — Convertir entre deux préfixes][
  On passe toujours par l'unité de base comme étape intermédiaire. Raccourci : l'exposant du résultat est la différence #blanc(w: 4.6cm).
]
...
#align(center)[
  #text(font: titlefont, size: 15pt, weight: "bold", fill: rgb("#5448c8"))[Pour aller plus loin]
]
...
#block(
  ...
  #grid(
    ...
    [
      #text(font: titlefont, weight: "bold", size: 11pt)[Révise et entraîne-toi chez toi]
      #v(4pt)
...
#rappel[Avant de partir, vérifie que tu sais…][
